# This Week's Mission: Order Processing & Settlement Engine

This is the business scenario that drives the rest of this week. Starting in
...
2. A client (or their advisor) submits an **order**: buy or sell a given
   ...
   once per day, from an upstream system (for this week: a CSV/text file).
...
- **Modules 10-11**: TDD fundamentals and JUnit, practised on pieces of this
  domain.
- **Module 12**: build core mission logic (order validation, holding updates) test-first,
  straight from the requirements.
- **Module 13**: wire everything together into a working engine (the settlement report and
  the orchestrating processing engine).
