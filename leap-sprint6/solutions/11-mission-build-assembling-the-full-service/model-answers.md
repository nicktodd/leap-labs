...
`OrderValidator.validate` (Sprint 5, unchanged) takes a `currentPortfolioValue` parameter meant to
...
market feed — a genuine feature this sprint never scoped. Using the single holding being traded,
...
The Sprint 3 schema's `asset_class` column stores `'Equity'`, `'Bond'`, `'Fund'`, `'Cash'`
(capitalised, from `enterprise-schema.sql`'s own `CHECK` constraint). `InstrumentFactory` (Sprint
5, unchanged) matches on `"EQUITY"`, `"BOND"`, `"FUND"` — uppercase, because that's what Sprint 5's
order files used. Neither side was "wrong" when it was written; they simply came from different
sprints with different conventions. `.toUpperCase()` here is the one-line adapter that lets two
...
Every module this sprint built a piece in isolation: Module 4 designed the shape of the URL,
