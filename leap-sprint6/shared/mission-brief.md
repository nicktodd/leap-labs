# Sprint 6 Mission: The Order Processing & Settlement Engine, as a Service

Sprint 5 built the Order Processing & Settlement Engine as a single Java program: read a batch of
orders from a file, validate them, execute them, print a settlement report. Sprint 6 turns that
same engine into a real, deployable microservice — the natural next step for a system whose logic
already works and is already fully tested.

You already have the hard part. `OrderValidator`, `HoldingUpdater`, the `Instrument`/`Feeable`
fee-calculation hierarchy, and `OrderProcessingEngine` (all from Sprint 5, Modules 7-13) do not
need to be rewritten — they need a service wrapped around them.

## What Changes, and What Doesn't

**Doesn't change:**
- The business rules (Sprint 5's `shared/mission-brief.md`, requirements 1-7)
...
No new business rules this sprint. If a design decision here conflicts with something Sprint 5
already decided, the Sprint 5 decision wins unless there's a specific architectural reason (HTTP,
persistence, security) that forces a change — and if so, that reason should be traceable, the
same way Sprint 5's Module 14 asked you to trace design decisions back to their cause.
