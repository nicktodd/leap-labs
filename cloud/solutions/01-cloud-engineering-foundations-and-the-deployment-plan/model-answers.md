...
| the auth service | Moves | ECS Fargate task, behind the same or a second ALB | Same pattern as `mission-service` - private subnet, fronted by a load balancer |
| Postgres | Moves | RDS | Private subnet only - never reachable from the internet, only from the ECS tasks that need it |
| Kafka | **Stays local** | N/A (Docker Compose) | Not part of this week's deployment |
...
scope for this week by deliberate decision, not an oversight.
...
**Applying this to your own capstone extension**: most extensions add one more thing that
