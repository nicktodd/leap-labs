...
architecture from the Spring, Kafka, Node, and Angular weeks to the whiteboard, not a laptop.
...
`mission-service` on `:8090`, the auth service on `:3000`, Postgres in a Docker
...
the MyBatis mappers - all of the Spring week's actual code ships unmodified. This week changes
*where it runs*, not *what it is*.

## Part 1: The Small Set of Services That Handle Most of the Work (10 min)

AWS has several hundred services. The mission needs six, and this week teaches exactly
...
| **RDS** | The `missionservice-postgres` Docker container |
...
is much larger (Lambda, SQS, DynamoDB, API Gateway, dozens more) - this week teaches the
...
Kafka (the Kafka week) is the one piece of the mission's architecture that deliberately does **not**
move to AWS this week. AWS has a managed Kafka service (MSK), but it's expensive to run
...
because "that's what this week is about."

## Key Message

Every AWS service this week teaches exists to do one job the mission's architecture already
needs done - nothing is being learned for its own sake. Before writing a single `aws` CLI
command, know what's moving, where, and why; the rest of the week is executing this plan.

## Transition to the Lab

In pairs, candidates map every piece of the mission's real architecture - `mission-ui`,
`mission-service`, the auth service, Postgres, Kafka - to where it lives in AWS (or
