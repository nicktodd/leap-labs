...
This week deliberately uses a small set of these services - S3, CloudFront, ECR, ECS, RDS,
...
resource this week creates belongs to exactly one region unless it's explicitly account-wide
...
first few times through a workflow. Everything this week automates from Module 11 onward uses
...
support message showing full output never leaks a working credential. Every command this week
...
completely inappropriate for a real production deployment credential. This week's later ECS
...
time for the rest of this week, especially once ECS task roles (Module 7 onward) replace this
