...
No laptop setup needed. Work in pairs, on paper or a whiteboard. Have the Spring, Kafka, Node,
and Angular weeks' architecture in front of you if you want to check specifics (`mission-service`'s
`application.yml`, the auth service's `main.ts`, `mission-ui`'s `environment.ts`).
...
- the auth service - NestJS, currently run with `node dist/main.js`
- Postgres - currently a Docker container (`missionservice-postgres`)
...
1. Does it move to AWS this week, or stay local? (One of the five has a deliberately
...
2. If it moves, which AWS service hosts it? Use only the six services from today's demo (S3,
   ...
   write down which one and why, but know that's outside this week's scope.
...
local `.env` file / `application.yml`). Where should each one live after this week, and why
...
If you were deploying your own capstone extension instead of the baseline mission,
