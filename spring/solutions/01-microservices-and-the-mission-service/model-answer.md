...
A JSON request body, roughly matching the Java week's `IncomingOrder` fields, plus the client
...
requirement the Java week never had to consider, because everything ran as one trusted process.
...
This maps directly onto the Java week's `ValidationResult` and `SettlementReport` - the shape of
the data barely changes, only how it's delivered (HTTP response instead of a report line).

## What does the service need to talk to, that the Java week didn't?

- **A database** (Postgres, the Data week's schema) - client, portfolio, and holding data can't live
