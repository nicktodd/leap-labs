...
**2. Was the Java week's logic really unchanged?** Yes - literally, byte-for-byte, other than the
package declaration (`com.neueda.leap.mission.engine` → `com.neueda.leap.mission.service.domain`) and one
added comment. `OrderValidator.java`, `HoldingUpdater.java`, and the `Instrument` hierarchy in
Module 11's `domain/` package are the exact classes from the Java week, Module 13. The mission
brief's claim held up under direct inspection, not just as a stated intention.
...
building real market-price lookups would need a live pricing feed this week never scoped. The
...
training-week trade-off from technical debt nobody flagged.
