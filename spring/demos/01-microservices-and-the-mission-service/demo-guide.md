...
Deliberately brief and motivating - this module's job is to set up the whole week's arc, not
...
network involved. The Java week's Order Processing & Settlement Engine was, structurally, a
monolith - small, but the shape is the same as a much bigger one.
...
**Land the honest conclusion explicitly**: neither is "better" as a default. The Java week's
engine was correctly a monolith - small, one team, no need for independent scaling. This week
has a concrete, specific reason to split it out (separate deployability, separate security
boundary, part of a larger system this cohort doesn't build) - and if a team can't name a reason
that concrete, that's usually a sign microservices is the wrong call for them, not a safe default.

## Open the Diagram

Show `monolith-vs-service.png`. Point at the Java-week box first - everything ran in one process,
one `main()` method, reading a file and printing to a console. Then point at this week's box: the
same core logic (`OrderValidator`, `OrderProcessingEngine`, `HoldingUpdater` - literally the same
classes) now sits behind an HTTP boundary, talks to a separate auth service, and persists to a
real database.

**Say explicitly: nothing about the business logic changed.** This is the whole point of the Java
week's SOLID work paying off a second time - the classes that were designed to not know about
their concrete collaborators don't care whether they're being called from a `main()` method or a
REST controller.

## Where the Java Week's Algorithm Becomes a Service Boundary

Walk through this week's half of the diagram slowly:
...
- **Everything else inside the box** - `OrderValidator`, `OrderProcessingEngine`,
  `HoldingUpdater` - is unchanged code from the Java week

## Transition to the Lab

In pairs, 15 minutes, whiteboard or shared doc - not a formal design, just enough to motivate
what's coming: sketch how the Java week's algorithm could become a service boundary. What comes in
