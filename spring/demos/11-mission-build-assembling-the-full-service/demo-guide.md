...
was looked up and updated against real Postgres rows (Module 7); was checked against the Java
week's `OrderValidator` - the exact same class, unmodified, that was tested against a CSV file
during the Java week; and would have produced a clean, structured error at ANY of those steps if
something had gone wrong (Module 10).
...
- `orderValidator.validate(...)`, `holdingUpdater.applyOrder(...)`, `instrumentFactory.create(...)`
  - from the Java week, Module 13, genuinely unchanged
...
properly needs a market-data feed this week never scoped. Say explicitly: this is a named
...
error handling, and all of the Java week's domain logic) given and already working - verified
against a pre-written integration test hitting the real Postgres container and the real running
auth stub.
