...
`InMemoryOrderRepository`). Today the service talks to the real Data week's Postgres schema - the
same `enterprise-schema.sql` learners already know from the Data week.

```bash
# Confirm the local mission database exists (created earlier in the programme, from
# the Data week's enterprise-schema.sql) - see the lab README's Setup section if not
...
```xml
<mapper namespace="com.neueda.leap.mission.service.HoldingMapper">
  <select id="findByClientId" resultType="com.neueda.leap.mission.service.Holding">
...
a bound parameter - the same SQL-injection protection the Data week covered for raw SQL. Contrast
