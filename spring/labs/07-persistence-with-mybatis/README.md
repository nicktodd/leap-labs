...
- Connected a Spring Boot service to the Data week's Postgres schema and returned live data

## Setup

- Java 21 and Maven installed
- Your local Postgres server from the Data week's Module 2, with a fresh `mission` database seeded with
  the enterprise schema:

  ```bash
  psql -U postgres -h localhost -c "CREATE DATABASE mission;"
  psql -U postgres -h localhost -d mission -f ../../../data/shared/enterprise-schema.sql
  ```

  (If you already created this database earlier, there's nothing to do - it's
  already seeded.)
- Given, don't modify: `MissionServiceApplication.java`, `Advisor.java`, `Transaction.java`,
  ...
  own local Postgres superuser credentials from the Data week, and update them if they don't
