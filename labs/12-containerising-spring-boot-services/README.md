...
- Written a multi-stage Dockerfile for the mission service (build stage separate from the
  runtime image)
- Run the containerised service on a Docker network alongside the Sprint 3 Postgres container,
  reaching it by container name rather than `localhost`
...
`sprint6-postgres:5432` — the container's **name**, and the **unpublished** internal port, not
the `localhost:5433` you've used everywhere else this sprint.
