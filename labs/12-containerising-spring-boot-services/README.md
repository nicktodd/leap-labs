...
Node/Express service that lives two levels up from this lab folder, at the root of
`leap-sprint6/` — already has a `Dockerfile`, so build and run it the same way you'll build and
...
By the end of Part E you have four containers running (`sprint6-postgres`, `auth-stub`,
...
flowchart TB
    ...
    subgraph host["Docker host (Linux VM or your Mac)"]
        subgraph missionnet["mission-net — user-defined bridge network"]
            ...
            postgres["sprint6-postgres<br/>Postgres 16, port 5432"]
            mission -->|"JDBC, by container name<br/>sprint6-postgres:5432"| postgres
        end
...
published port) or from `mission-service-lab` to `sprint6-postgres` (via `mission-net`).
...
- **Placing an order** (`curl … localhost:8081/accounts/1/orders`) reaches
  ...
  2. Only once that check passes does `mission-service-lab` talk to Postgres — and it does that
     over `mission-net`, by container name (`sprint6-postgres:5432`), the same way any two
...
`mission-service-lab` and `sprint6-postgres` genuinely exchange data (SQL over JDBC) on every
