...
- Reused Sprint 5's `OrderValidator`, `HoldingUpdater`, and fee-calculation hierarchy completely
...
- The shared auth stub running (leave it running for the whole lab):
  ...
  ```
- Given, don't modify: everything in `domain/` (Sprint 5's business logic, unchanged),
...
4. Build a `domain.OrderRequest` from the DTO and call `orderValidator.validate(...)` — Sprint 5's
   `OrderValidator`, completely unchanged. If the result isn't valid, throw
