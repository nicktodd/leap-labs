...
```bash
mvn compile
mvn exec:java -Dexec.mainClass=com.neueda.leap.mission.stream.PartitionKeyChoice
```
...
but the actual trading platform from the Spring week - naming the topic(s), choosing the partition key,
