...
```bash
docker run -d --name sonarqube-missionstream -p 9000:9000 sonarqube:community
```
...
```
QUALITY GATE STATUS: PASSED - View details on http://localhost:9000/dashboard?id=missionstream-starter-codebase
BUILD SUCCESS
```
...
**Quality Gates → Create** → name it `Mission Stream Strict Gate`, add conditions:
- `Bugs` is greater than `0` (on Overall Code, not just New Code)
- `Reliability Rating` is worse than `A` (on Overall Code)

Assign it to the project: **Project Settings → Quality Gate → Mission Stream Strict Gate**.
...
```
QUALITY GATE STATUS: FAILED - View details on http://localhost:9000/dashboard?id=missionstream-starter-codebase
BUILD FAILURE
