...
- Postgres (the Data Systems week's enterprise schema, reused for batch loading - see `shared/`)
...
This week runs two threads in parallel. The trading platform (the Spring week's mission service)
gains a real-time event stream - every accepted order also publishes a trade event to Kafka, so
any number of downstream consumers can react without polling or coupling to the mission service
...
real codebase rather than isolated toy examples. Both threads meet in Module 14, where the Python
week's analytics dashboard is extended to consume from both the new Kafka stream and a
batch-loaded warehouse table. See `shared/mission-brief.md`.
...
| 14 | Mission Build: Batch + Real-Time Dashboard Integration & Wrap-up | [labs/14-mission-build-batch-and-real-time-dashboard-integration/README.md](labs/14-mission-build-batch-and-real-time-dashboard-integration/README.md) |
