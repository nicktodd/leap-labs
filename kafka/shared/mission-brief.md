# This Week's Mission: The Trading Platform Gets Real-Time, and the Old Code Gets Honest

This week has two missions running in parallel, meeting at the end in Module 14.

## Mission A - Real-Time: Kafka for Trade Events

The trading platform (the mission service, built across the Spring week) currently only knows
about orders the moment they're submitted. Nothing downstream - reporting, the analytics
dashboard, other services that care when a trade happens - finds out unless they poll Postgres
themselves.

This week adds an event stream: every accepted order also publishes a trade event to Kafka. Any
number of independent consumers can subscribe without the mission service knowing or caring who
they are - the same decoupling principle the Spring week's JWT validation already taught (the
mission service doesn't call the auth service; here, it doesn't call its consumers either, it
just publishes).

## Mission B - Honesty: the Refactor Codebase

Alongside the new work, this week ships a **starter codebase** - a real, running, but visibly
...
The mission's analytics dashboard (introduced in the Python week) gets extended to consume from **both**
...
**Doesn't change:**
- The mission service's core order-processing logic (from the Spring week) - Kafka publishing is
  an addition to it, not a rewrite of it
- The Python week's dashboard's existing batch-loaded view
