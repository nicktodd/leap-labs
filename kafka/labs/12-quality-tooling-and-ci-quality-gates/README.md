...
Reuse the demo's SonarQube container (`sonarqube-missionstream`) and the `Mission Stream Strict Gate` created
...
`missionstream-m12-lab` (or query `/api/issues/search?componentKeys=missionstream-m12-lab`) and read the
...
A fixed `SettlementReporter.java` that passes the `Mission Stream Strict Gate` with `bugs: 0`,
