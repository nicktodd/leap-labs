...
Reuse `kafka-missionstream` and the `trade-events` topic from the demo (run the demo's `OrderService`
...
```bash
java -cp "target/classes;$(cat cp.txt)" com.neueda.leap.mission.stream.FraudDetectionConsumer
```
