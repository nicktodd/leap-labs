...
- **TODO 2**: the correct order is sync code first (`sync A`, `sync B`), then
  ...
  (the standard microtask queue), then `setTimeout` (a macrotask) — matching Module 8's
...
- `require("./count-utils.cjs")` has no extension requirement; Module 6's `NodeNext`
...
This is genuinely more than a syntax swap — it's why Module 6 spent an entire module
