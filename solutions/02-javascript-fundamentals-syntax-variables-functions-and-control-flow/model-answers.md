...
unfixed through Module 3's refactor (which changes *how* the data is represented, not the
lockout logic itself) — a genuine example of a code smell being visible long before anyone
decides it's worth the cost to fix it, echoing Sprint 7's "technical debt" framing from Module 1.

## Talking points

- `parts[0]` / `parts[1]` is deliberately verbose — Module 3 introduces array destructuring
