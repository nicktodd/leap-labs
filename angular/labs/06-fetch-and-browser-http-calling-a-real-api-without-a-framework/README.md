# Lab 6 - Fetch & Browser HTTP: Calling a Real API Without a Framework
...
1. **This week's auth stub** (`shared/auth-stub`) on `http://localhost:3000`, with CORS
...
2. **Invalid credentials** - `alice` / anything else (8+ characters, so the client-side
   ...
   not from `catch`.
3. **Network failure** - stop the auth stub (or temporarily re-comment the `cors()` line),
   then submit. A red message should still appear, but this time it's
