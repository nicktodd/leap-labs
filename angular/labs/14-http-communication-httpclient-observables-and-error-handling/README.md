# Lab 14 - HTTP Communication: HttpClient, Observables & Error Handling

## Setup

Your own `mission-ui` from Module 13, with the `Holdings` service and `PortfolioBadge`
...
1. Click "Load From API" (or whatever you named it). The two Module 12/13 seeded holdings
   should be **replaced** by the mock server's real response (`VOD.L`, `HSBA.L`) - and the
   header badge from Module 13 should update too, with zero code connecting the two components
   directly.
...
`.subscribe()` on it. Module 6's `fetch()` sends its request the moment it's called, `await`
