# Module 9 Lab - The Full Stack So Far & Where Angular Fits
...
- Identified exactly which existing piece each future Angular module will build, and
  which parts of Modules 1-8's hand-written HTML/CSS/JS/TypeScript each one replaces
...
A trader opens `mission-ui` (the Angular app you'll spend the rest of this week
building), logs in, and submits an order. Modules 1-6 already built a rough version of
"logging in" by hand - a static HTML form, some CSS, and a `fetch()` call to the auth stub. Today's demo proved every box past that point (the mission
...
1. What does the trader type into the Angular app, and what does Angular send to the
   auth stub, and with what?
...
A diagram with five boxes - Angular app, the auth stub, mission service,
Postgres, and the trader's browser session/token storage - arrows labelled with what's
actually sent at each hop, covering both scenarios. Next to each box, write which module
builds it (or "already built, an earlier week" for the two that aren't new).
...
- Each of the five boxes is labelled with the module (or prior week) that owns it
