# Lab 16 - OpenAPI-Generated API Clients

## Setup

Your own `mission-ui` from Module 15, plus the same running mission service, auth stub,
and `missionservice-postgres`. Add `springdoc-openapi-starter-webmvc-ui` (version `2.6.0`) to the
...
1. Query Postgres before, exactly as Module 15.
...
`HttpClient` call (Module 14/15's style) never have this specific failure mode - what is it
...
controller's `OrderRequestDto` gains a new required field later, and nobody regenerates
