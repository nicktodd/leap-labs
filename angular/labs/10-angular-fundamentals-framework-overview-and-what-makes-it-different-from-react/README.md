# Lab 10 - Angular Fundamentals: Framework Overview & What Makes It Different from React

## Format

A 20-minute reading exercise, in pairs. No project to run, no npm install - that's Module 11.
...
1. **Already known** - something Modules 1-6 (events, reading/updating something on screen),
   Modules 7-8 (TypeScript, classes, interfaces), or today's Part 0 (decorators) already
   covered, just with Angular's specific syntax around it.
...
- Whether `holdings` (a signal) or `Holding` (the interface) is the Angular-specific concept,
  and which one is just an ordinary TypeScript type Modules 7-8 already covered
...
1. Is the *decorator syntax itself* (`@Something(...)` above a class) Angular-specific, or is
   it a general TypeScript feature Angular happens to use? (Hint: look back at today's
   Part 0 `@Logged` example.)
...
Full answers, with reasoning: `solutions/10-.../model-answers.md` - check it after your own
pass, not before.
