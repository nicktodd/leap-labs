# Lab 17 - Reactive Forms: Building, Submitting & Handling Responses

## Setup

Your own `mission-ui` from Module 16, with the generated `OrderControllerService` and
`MissionApi` working. Same running backends as Module 15/16.
...
5. In `mission-api.ts`'s `catchError`, change the error handling to read `error.error?.message`
   (the real response body from the Spring week's `GlobalExceptionHandler`) before falling back to
   `error.message`.
...
4. Query Postgres before and after your successful order, the same way Module 15/16 did -
...
specifically referencing what Module 5's `novalidate` discussion already established about
