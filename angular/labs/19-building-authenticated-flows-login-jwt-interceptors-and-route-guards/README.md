# Lab 19 - Building Authenticated Flows: Login, JWT Interceptors & Route Guards

## Setup

Your own `mission-ui` from Module 18. Same running backends as Modules 15-18 (mission
service, auth stub, `missionservice-postgres`).
...
3. Generate `ng generate service auth-api` with one method, `login(username, password)`,
   posting to the auth stub's `/auth/login` and returning the response.
4. Generate `ng generate component login-page --standalone`. Build a reactive form (Module
   17's pattern) with `username`/`password` fields. On submit, call `AuthApi.login()`; on
...
7. Remove any manual login/credential-setting code left over from Module 15/16 in
...
10. Add a `/login` route (lazy-loaded, same as every other route this week).
...
4. Query Postgres before and after, the same way every module since 15 has, to confirm the
