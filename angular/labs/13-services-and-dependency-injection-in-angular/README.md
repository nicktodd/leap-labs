# Lab 13 - Services & Dependency Injection in Angular

## Setup

Your own `mission-ui` from Module 12, with the `HoldingsSummary` component working.
...
2. Move `holdings`, `totalValue`, and the add/remove logic out of `HoldingsSummary` and into
   ...
   - Keep `totalValue` as a `computed()`, unchanged in spirit from Module 12.
   - Turn `addHolding`/`removeHolding` into plain methods (`add`/`remove`) that call
     `.update()` the same immutable way Module 12 did.
...
4. Rebuild and rerun. Confirm the component behaves *identically* to Module 12 - this step is
