# Module 4 Demo Guide - Working with Objects, Arrays & Modern JavaScript

This module takes Module 3's `login-attempts.js` and rebuilds it with what real-world
...
Everyone in the room already knows Java (the Java week) and Python (the Python week). Every new
...
Module 3 already used both without naming them: `parseAttempt` returned an object
...
Compare this to Module 3's `parseAttempt`, which did the array-position version
...
A third way to write a function, on top of Module 3's declaration and expression
forms:
...
Arrow functions are **function expressions** under the hood - Module 3's rule still
...
These aren't required for this module's lab, but they're worth a mention: Module 4's
...
same `import`/`export` syntax used throughout TypeScript and NestJS later in the Node week.
...
- `summarize` - same destructuring-in-a-loop idea as Module 3, now over objects
...
Same behaviour as Module 3's script, same `bob` lockout result - because it's genuinely
...
Learners take Module 3's `login-attempts.js` (the string-parsing version) and refactor
