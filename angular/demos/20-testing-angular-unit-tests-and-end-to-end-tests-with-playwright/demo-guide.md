# Module 20 Demo Guide - Testing Angular: Unit Tests & End-to-End Tests with Playwright

**Duration:** 30 minutes
**Prerequisite:** Module 19's login flow. Same running backends. `@playwright/test` installed
as a dev dependency (`npm install --save-dev @playwright/test`).

Every module this week has been verified by hand - a screenshot, a Postgres query, a
...
default scaffold's "Hello, mission-ui" text - Module 11 changed `app.html` to "Mission Control"
...
`TokenStore` (Module 19) is a good unit-test target: pure logic, no HTTP, no DOM - exactly
...
redirects. Each of those pieces was designed to be testable in isolation (Module 19's whole
...
this entire week - the difference is that it's now a committed file, run with one command,
...
Learners write one meaningful unit test of their own (for a component or service this week
