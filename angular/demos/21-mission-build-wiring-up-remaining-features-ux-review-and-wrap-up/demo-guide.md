# Module 21 Demo Guide - Mission Build: Wiring Up Remaining Features, UX Review & Wrap-up

**Duration:** 30 minutes
**Prerequisite:** Module 20's test suite. Same running backends.

## Part 0: A Gap That Twenty Modules of Testing Didn't Catch (5 min)

Module 20 built a whole test suite - unit tests, an end-to-end suite - and it all passes. That
...
number. `Holdings` (Module 13's service, still holding its original two hardcoded rows) had no
idea an order had even been placed. This is exactly the kind of gap Module 20's tests don't
...
enough for both to update correctly, which is the whole reason Module 13 put holdings behind a
...
Module 20's whole suite still passes unchanged - the fix touched `MissionApi` and `Holdings`,
neither of which either test suite directly asserts against, which is itself worth noting: the
suite catching a broken guard in Module 20 doesn't mean it catches every kind of gap, only the
kind it was written to look for.

## Part 5: UX Review - What Twenty Modules Left Unfinished
...
- The holdings list still has "Add BP.L", "Load From API", and "Simulate API Error" buttons
  left over from Modules 12 and 14's demos - useful for teaching, not for a real user.
- There's no way to view past orders - `lastOrder` shows only the most recent one, then it's
  gone on refresh (deliberately, per Module 19 - no persistence layer was ever built).
- Form validation messages (Module 17) only appear after a field is touched - correct
...
Every module this week tested one thing in isolation and verified it worked. Nothing until
