# Module 11 Demo Guide - Angular Project Structure, Dev Server & Build Tooling

**Duration:** 25 minutes
**Prerequisite:** Module 10's conceptual read-through. Node.js 22.22.3+ or 24.15.0+ and the
Angular CLI (`npm install -g @angular/cli`, or use `npx @angular/cli` as this demo does).
This week targets **Angular 21** and **TypeScript 5.9** specifically, not whatever's
newest - pin the CLI version explicitly (`npx @angular/cli@21`) rather than `@latest`.

Module 10 read Angular code without running it. Today scaffolds a real, running project for
...
scaffolds `app.routes.ts` up front (Module 18 fills it in); `--style=css` picks plain CSS,
the same language Module 2 already taught; `--ssr=false` keeps this a pure browser app, no
server-rendering complexity this week needs.
...
├── tsconfig.json          - config for the language Module 7 introduced, Angular-flavoured
├── src/
...
│   └── app/
│       ├── app.ts          - the root component (Module 10's MissionStatusComponent, for real)
│       ├── app.html        - the root component's template
│       ├── app.css         - the root component's own styles
│       ├── app.config.ts   - app-wide providers (router, HTTP client - Module 14)
│       └── app.routes.ts   - the route table (empty until Module 18)
└── public/favicon.ico
```

Point at `src/app/app.ts` - this is Module 10's own pattern, generated for real:
...
Two things worth naming explicitly against Module 10's version: current Angular CLI no longer
...
verify it in a browser - the actual project this week's remaining modules build inside,
starting today.
