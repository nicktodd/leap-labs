# Module 15 Demo Guide - Connecting to the Spring Boot Backend: First Real API Call

**Duration:** 25 minutes
**Prerequisite:** Module 13's DI pattern, Module 14's HttpClient/catchError pattern. A working
checkout of the Spring week's mission service, `missionservice-postgres` running, and this
week's auth stub - the same three pieces Module 9's whiteboard exercise already traced by
hand.

Module 14 called a mock server built for this week alone. Today calls the **real** thing -
...
Two one-line additions, on the *server* side, mirroring Module 6's own CORS fix:
...
```javascript
// server.js, this week's auth stub (Module 6's own fix, origin list extended)
app.use(cors({ origin: ['http://localhost:8000', 'http://localhost:4200'] }));
```

Same lesson as Module 6: CORS is enforced by the browser, fixed on the server, never worked
...
`catchError` and `of(null)` are exactly Module 14's pattern, unchanged. The new operator is
...
deliberate stand-in: Module 19 builds the real login form, JWT interceptor, and route guard
...
Nothing new here at all - `inject()` (Module 13), signals read straight from the service
(Module 13), a button calling a method (Module 12). The entire point of today is that the
...
`503` → `504` - not asserted, queried twice, before and after, exactly like Module 9's own
...
Nothing about *how* Angular talks to a backend changed between Module 14's mock and today's
...
that the Data Systems, Java, and Spring weeks already built and proved, plus this week's own
auth stub, one `curl` command at a time. Today is the first time an Angular click reaches
all the way through to a real database row.
