# Module 18 Demo Guide - Routing Fundamentals: Routes, Lazy Loading & Navigation

**Duration:** 25 minutes
**Prerequisite:** Module 17's `HoldingsSummary` and `PlaceOrder` components, both currently
...
`PlaceOrder`'s reactive-forms code (Module 17) doesn't ship to a visitor who only ever looks
at their holdings.
...
idea Module 5's `preventDefault()` established, now built into the framework). `[routerLinkActive]="active"`
