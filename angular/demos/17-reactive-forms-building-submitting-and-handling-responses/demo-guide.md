# Module 17 Demo Guide - Reactive Forms: Building, Submitting & Handling Responses

**Duration:** 30 minutes
**Prerequisite:** Module 16's `PlaceOrder` component and generated `OrderControllerService`.
Same running backends as Modules 15/16.

Every module since Module 15 has submitted the *same* hardcoded order with a single button.
...
no errors at all. Valid submissions flow straight into Module 16's generated client,
unchanged.
...
`cannot sell more than the current holding` - this is the Java week's `OrderValidator`,
unchanged,
...
`HttpClient` puts the actual **response body** on a failed request; the Spring week's
