# Module 19 Demo Guide - Building Authenticated Flows: Login, JWT Interceptors & Route Guards

**Duration:** 35 minutes
**Prerequisite:** Module 18's routed `/holdings` and `/orders`. Same running backends as
Modules 15-18. Every earlier module hardcoded `alice`/`mission123` inside `MissionApi` - today
...
Module 13's exact pattern - a service, `providedIn: 'root'`, shared by anything that injects
...
Module 17's reactive-form pattern exactly, calling `AuthApi.login()` (a thin service wrapping
one `HttpClient.post` to the auth stub's real `/auth/login`). Success stores the token and
navigates away; failure shows a message - nothing new here that Modules 14 and 17 didn't
already teach.
...
This replaces every earlier module's manual header-attaching. Module 15 set a header on one
`http.post` call by hand; Module 16 set `configuration.credentials` once, manually, right
