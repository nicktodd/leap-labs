# Module 14 Demo Guide - HTTP Communication: HttpClient, Observables & Error Handling

**Duration:** 30 minutes
**Prerequisite:** Module 13's `Holdings` service and `PortfolioBadge`. Angular 21, TypeScript
5.9. This demo's `mock-server.js` running (`node mock-server.js`, no dependencies) - a real
mock API, not the Spring week's backend yet, that's Module 15.

Module 6 called a real backend with `fetch()`. Today does the same job with Angular's own
...
Run it (`node mock-server.js`), curl both routes live. This is deliberately *not* the Spring
week's Spring Boot service - Module 15 connects to that. Today isolates the one new idea:
...
Like `provideRouter` (Module 11), `HttpClient` has to be registered once at the application
...
Compare directly to Module 6's `fetch()`:
...
- **`.subscribe()` instead of `await`** - `HttpClient`'s methods return **Observables**, not
  ...
  the Promises Module 6 used: a Promise starts running the moment it's created; an Observable
...
The two seeded holdings from Module 12/13 are **replaced** by the mock server's real response -
`VOD.L` and `HSBA.L`, exactly what `mock-server.js` returns. The header badge (Module 13's
...
`HttpClient` solves the exact same problem `fetch()` did in Module 6 - send a request, handle
