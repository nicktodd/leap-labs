# Module 6 Demo Guide - Fetch & Browser HTTP: Calling a Real API Without a Framework
...
The Java and Spring weeks built the mission service. This week's auth stub protects it.
Modules 1-5 built a page that can validate its own form. Today that page talks to a real,
...
inside it. This is the exact same `async`/`await` syntax used anywhere else in JavaScript,
...
## Part 3: The Fix Is Two Lines, on the Server (2 min)

In `shared/auth-stub/server.js`, uncomment:
...
`cors()` adds the `Access-Control-Allow-Origin` header the browser was checking for.
...
The token shown is real, truncated only for display - a genuine signed JWT the mission
service will accept unchanged, once Module 19 calls it directly.
...
the failure modes a real network actually has. Module 14's `HttpClient` will do the same job
...
Learners take Module 5's own login page and connect it to the real auth stub: a `fetch()`
call with `async`/`await`, a `response.ok` check for
