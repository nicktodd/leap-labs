# Module 3 Demo Guide - JavaScript Fundamentals: Syntax, Variables, Functions & Control Flow

First JavaScript of the week. No frameworks, no build step, no npm install - just `node` and
...
genuinely Module 4 content, appearing one module early. Say so directly: today's task doesn't
require understanding objects deeply, only that a function can bundle more than one return value
together. Module 4 revisits this exact function and makes it more idiomatic.
