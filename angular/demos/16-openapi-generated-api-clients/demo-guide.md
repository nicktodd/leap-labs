# Module 16 Demo Guide - OpenAPI-Generated API Clients

**Duration:** 25 minutes
**Prerequisite:** Module 15's `MissionApi`/`PlaceOrder`, hand-written against the real mission
service. Same running backends as Module 15.

Module 15 wrote the `POST /accounts/{accountId}/orders` call by hand - the URL, the body
...
directly with the Spring week's Module 5 `order-service.yaml` - a **hand-written**, contract-first
...
fixed on the server, the same way Module 6 and Module 15 fixed real CORS gaps:
...
## Part 3: Replacing Module 15's Hand-Written Call (7 min)
...
`this.orderApi.submitOrder(...)` entirely replaces Module 15's manual
...
Identical real behaviour to Module 15 - same backend, same database, same accepted order -
...
hand-written call (Module 15) would never have hit, because a hand-written call never had to
...
Learners generate their own client from the same live spec, and replace their own Module 15
