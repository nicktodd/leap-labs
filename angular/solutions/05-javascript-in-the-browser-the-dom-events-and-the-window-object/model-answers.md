...
- Fixing both fields and resubmitting produced `Welcome, nicktodd. (No real request sent
  yet - Module 6 adds fetch().)`, and `#username`'s value was empty afterwards - confirming
...
native version can only show one message at a time, styled entirely by the browser (Module 1's own earlier screenshot showed the native bubble covering the password field). The
...
way from Module 6 onward: a `fetch()` failure can reuse `showMessage(..., "error")` unchanged,
