...
- `npx ng test --watch=false` before any changes: **1 failure** - `app.spec.ts` expected
  "Hello, mission-ui", the CLI's default scaffold text, which Module 11 replaced with
...
- **The stale `app.spec.ts` assertion**: a real, unplanned finding - not a contrived example.
  It sat unnoticed since Module 11 because nothing ran the test suite in between.
...
- useful for CI environments where standing up a full Postgres + Spring Boot + auth stub
stack per test run is expensive. What's lost is coverage of the actual integration: a mock that
...
mission-service `produces` bug Module 16 found. Today's suite caught a real, undiagnosed
