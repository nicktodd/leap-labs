...
- **`provideHttpClient()`**: registered once in `app.config.ts`, the same pattern as
  ...
  individually.
- **`inject(HttpClient)`**: identical pattern to `inject(Holdings)` from Module 13 - DI isn't
...
- **`.subscribe()`**: nothing happens - no request is even sent - until this is called. This
  is the core Observable-vs-Promise distinction Module 6's `fetch()` didn't have to teach.
...
error - the code just quietly does nothing. Module 6's `fetch()` can't have this exact bug:
