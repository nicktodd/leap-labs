...
## Already Known (Modules 1-9 / Today's Part 0)
...
- `interface Holding { ticker: string; quantity: number; }` - an ordinary TypeScript
  interface, exactly as Modules 7-8 covered.
- `export class HoldingsListComponent { ... }` - an ordinary exported class.
- `private router = ...` - a typed class field, ordinary TypeScript syntax by this point in
  the course.
...
- The *decorator syntax itself* - `@Component({...})` sitting above a class is the same
  TypeScript decorator feature today's Part 0 `@Logged` example used. What's Angular-specific
  is which decorator (`@Component`) and its config shape, not decorators as a language
  feature.
- `{{ holding.ticker }}` reading a value onto the screen and `(click)="goToOrders()"`
  handling a click - conceptually the same "read a value" / "handle an event" ideas Module 5
...
- `this.router.navigate(['/orders'])` - Angular Router's own API for a client-side
  navigation (no full page reload, similar in spirit to Module 5's `preventDefault()` but
  handled by the framework).

## Resolving the Two Disagreements

1. **Decorator syntax vs Angular-specific**: the syntax (`@Name(...)` above a class) is a
   general TypeScript feature - today's Part 0 `@Logged` example used it identically. What's
   new is *this specific decorator*, `@Component`, and the shape of object it expects.
