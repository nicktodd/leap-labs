...
contain. Modules 1-6 already built a rough, hand-written version of Scenario A (a
...
1. **Angular sends**: the username and password the trader typed into a login form
   (Module 19), as a `POST` to the auth stub's `/auth/login` - the exact same
   request body Module 6's `fetch()` call sent, and today's demo sent again with
   `curl`.
2. **Comes back and is stored**: `{ accessToken, refreshToken }`. The Angular service
   layer (Module 13) holds the access token in memory (a signal or a service field, not
   `localStorage` by default - worth a brief tangent on XSS risk if a pair proposes
   `localStorage`), so a JWT interceptor (Module 19) can attach it to every later
   request without every component needing to know it exists.
3. **While in flight / on failure**: a loading indicator (Module 21), and on a `401`
...
1. **Angular sends**: the order form's values (Module 17's reactive form), as a `POST`
   ...
   Module 19's interceptor, not typed by hand anywhere in the component.
2. **Signature check**: the mission service (`SecurityConfig`, unchanged since the
   Spring week). **Never sees the password**: the mission service - only the auth stub
   ever compares a password to a hash. **Never sees raw SQL**: the mission service's own
   controller code - MyBatis mappers (from the Spring week) parameterise every query
   underneath it.
3. **On `401`/`403`**: distinct messages - `401` means the interceptor's token is
   missing or expired (Module 19's guard should have redirected to login before this
...
- **Angular app** (this week) - initiates login once, attaches the token to
...
- **The auth stub** (built fresh this week) - the only box that ever sees a password.
  Called once per login, not once per request.
- **Mission service** (from the Java and Spring weeks) - checks the token's signature and
  the trader's role on every request; queries Postgres via parameterised MyBatis mappers.
  Never checks a password, never calls the auth service back.
- **Postgres** (from the Data Systems week) - stores the real data. Only ever reached
  through the mission service.
- **Token storage** (this week, Module 19) - the one genuinely new piece of state this
  week introduces: something has to hold the token between the login response and the
  next request, and that something is Angular application state, not a cookie the
  browser manages for you by default. Modules 1-6 got away without this entirely - a
...
- If a team's diagram shows the interceptor as a manual step inside every component
  ("component reads the token, adds the header"), that's worth flagging now: Module 19
  ...
  it's also exactly the kind of repetitive-by-hand code Module 6's `fetch()` call
