...
Run for real against the actual Spring week mission service (CORS-enabled, port 8090 in
this setup), this week's auth stub (CORS-enabled, port 3000), and `missionservice-postgres`:
...
- **CORS on two different servers**: the mission service (Spring Boot) and the auth stub
  (Express) each needed their own CORS fix - there's no single place this gets configured
...
- **Nothing new in the component**: `PlaceOrder` uses exactly Module 13's `inject()` pattern
  and Module 12's signal-reading pattern - proof that a service's *internals* can grow
...
exactly the reason Module 6 raised: anything in `script.js`/`.ts` ships to every visitor's
browser. It's acceptable *here* specifically because this module's whole scope is deliberately
narrow - "prove the mission service call itself works" - and Module 19 is explicitly
