...
Ran headlessly with Playwright against a real, running auth stub
...
- **TODO 4**: `data.accessToken.slice(0, 20)` is purely a display choice - the full token
  is already sitting in `data.accessToken`, unslic­ed, ready for Module 19 to actually
  store and use it.
...
real application (or Module 14's `HttpClient`-based Angular services) often needs to react
...
the auth stub's own `JWT_SECRET`). The username/password typed into this form are
