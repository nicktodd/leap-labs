...
def extract():
    """Extract: get the raw data from its source, unchanged."""
    return pd.read_csv(BASE / "messy-trades-raw.csv")


def transform(df):
    """Transform: Module 6's cleaning steps, as a single reusable function."""
...
def load(df, out_path):
    """Load: write the clean result somewhere downstream analysis can rely on."""
...
if __name__ == "__main__":
    ...
    load(clean, out_path)
    print(f"Extracted {len(raw)} raw rows, transformed to {len(clean)} clean rows, loaded to {out_path.name}")
