...
missing, and understand what this sprint replaces before writing a single line of code.
...
Open `shared/auth-stub/server.js` (Sprint 6/7) together and read it aloud:
...
This has worked, genuinely, for two sprints — because its only job was to prove `SecurityConfig`
...
- **A credential store** — where real usernames, real hashed passwords, and real roles actually
  live (today: a JavaScript object; by Module 12: a real Postgres table)
...
This is *why* the mission service's `SecurityConfig` was built the way it was in Sprint 6: not
...
sprint builds, to a protected mission-service route — before any of that code exists. The
sketch from today becomes the target Module 15 actually builds toward.
