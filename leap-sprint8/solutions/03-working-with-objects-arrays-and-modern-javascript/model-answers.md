...
per-user, sequence-sensitive logic as Module 2, just applied to this lab's dataset.
...
- **TODO 2** combines two Module 3 ideas in one line: destructuring `{ username, outcome }`
...
- **TODO 3**: `checkLockouts` is given as the rest-parameter example; `isLockedOut` is a
  plain per-user loop, identical in spirit to Module 2's `while` loop, just scoped to one
...
introduced: map each attempt to its `username` (an idea Module 4 formalises further with
