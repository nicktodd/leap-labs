# Demo: Module 04 - Containerising the Project Skeleton

**Duration:** 12 minutes
**Prerequisite:** A team repository already set up with a branching strategy (the Foundations
week's Module 9). Docker Desktop running, and access to this week's Jenkins instance. No new
commands beyond the Foundations week's Module 4, and this week's Modules 02 and 03.

## Part 1: Framing the task (1 min)

Narration: like the Foundations week's Module 9, there's nothing new to learn here technically.
This is Module 3's Dockerfile skills, the Foundations week's Module 4 branching skills, and
Module 2's Jenkinsfile syntax, applied together, on the team repository you just created.

## Part 2: Branch, add the skeleton (3 min)

Using whatever branch naming the team agreed in the Foundations week's Module 9 (or a sensible
default like `feature/docker-skeleton` if their strategy doesn't specify one):
...
nothing to leave running or inspect afterwards, unlike Module 3's app.
...
Narration: this is exactly the moment the team's Module 9 decision matters. If they chose
trunk-based, merge straight back to `main` quickly. If they chose something with a review step,
have a teammate look at the branch first. Either way, the mechanics are identical to the
Foundations week's Module 4.
...
reproduce the same containerised skeleton from a clean checkout. That's the property the
Jenkins job we build in Part 5 will verify automatically.
...
do that reproduction check automatically, the same discipline from Module 1, now applied to the
