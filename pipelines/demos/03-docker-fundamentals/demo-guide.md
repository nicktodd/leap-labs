# Demo: Module 03 - Docker Fundamentals
...
`pipelines-greeter-app-0.1.0.jar` from the `starter/` project (run `mvn clean package` first).
...
fill the gap yourself, same habit as the Foundations week's Module 8 and this week's Module 02.
...
WORKDIR /app
COPY target/pipelines-greeter-app-0.1.0.jar app.jar
ENTRYPOINT ["java", "-jar", "app.jar"]
```

```bash
docker build -t pipelines-greeter-app .
docker run -d --name greeter pipelines-greeter-app
docker ps
docker logs greeter
