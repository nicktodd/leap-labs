# Demo: Module 02 - Introduction to Jenkins

**Duration:** 12 minutes
**Prerequisite:** Access to this week's Jenkins instance. A Pipeline job called
`pipelines-demo-app` already created, pointing at a repository containing the
[`starter/`](../../labs/02-jenkins-intro/starter) project from this module's lab (this is the
...
- **Dashboard**: the list of all jobs, with status icons (blue/green ball = last build passed,
  red = failed)
- Click into `pipelines-demo-app`: the **job page** shows build history down the left, a
...
Narration: everything here mirrors what you saw in Module 01's sample log, just inside the
...
the code it builds. That's what we're using this week, and what you saw the syntax of in
Module 01.
...
same way as the Foundations week's Module 8: does this match what we'd expect, and is there a way to confirm it (for
...
habit built in the Foundations week's Module 8.
