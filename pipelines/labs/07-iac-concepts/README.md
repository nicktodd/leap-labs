...
Full hands-on cloud work comes later in the programme, so this part is optional and only for
...
2. Configure sandbox credentials, ideally a named AWS CLI profile rather than raw environment
   variables:
   ```bash
   aws configure --profile pipelines-sandbox
   export AWS_PROFILE=pipelines-sandbox
   ```
