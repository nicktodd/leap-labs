# Module 12 Demo Guide — Final Capstone Preparations & Sprint 11 Wrap-up

**Duration:** 50 minutes
**Prerequisite:** Every earlier module in this sprint, plus each candidate's own Sprint 10
capstone extension.

## Part 0: What This Module Is (5 min)

Modules 1-11 deployed the *baseline* mission — the shared trading platform every candidate
started Sprint 10 with. The mission brief has said from the start that this sprint's deployment
steps apply to a Sprint 10 capstone extension exactly as they apply to the baseline: same
...
covering three things: the trading platform, the Sprint 10 extension, and **how Copilot was used
...
taught, applied to the whole sprint at once, not just one module's resources:
...
- Either way, the Acceptance Tests stage should grow to cover the capstone feature specifically,
  not just Sprint 9's original login flow — a new Playwright spec, or an extension of the existing
...
which module did Copilot's suggestion need correcting, and how was that caught? This sprint's own
working pattern is the example to point to: several modules this sprint hit a real AWS error,
...
The whole sprint has been building toward this: not a new deployment target, but the same set of
