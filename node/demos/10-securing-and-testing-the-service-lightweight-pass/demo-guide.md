# Module 10 Demo Guide - Securing & Testing the Service: Lightweight Pass

**Duration:** 10 minutes
**Prerequisite:** Module 9's `AuthService` (register/login/refresh, bcrypt, JWT
...
`auth.service.ts`'s own `JWT_SECRET` line, already on screen from Module 9:
...
validates the signature via `jwt.verify` (the exact call Module 9's demo used) and
...
`assertThrows` from the Java week if anyone asks how this compares.
...
real Jest suite, rather than chasing exhaustive coverage this late in the week.
