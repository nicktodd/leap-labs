...
1. **Client sends**: a username and password (eventually, in Module 7, to
   the auth service's `POST /login`) - exactly the same shape as today's demo's call to
   `shared/auth-stub`.
2. **The auth service checks**: does a user with that username exist, and does the submitted
   password match the STORED, HASHED password (Module 8 - never a plaintext comparison like
   today's stub does).
...
- **Mission service**: verifies signatures and enforces per-endpoint role checks. Does NOT store
  ...
  holder logged in.
- **Postgres**: stores real user records (Module 8 onward) and the mission service's own
...
- The most common mistake in this sketch: an arrow from the mission service back to the auth
  ...
  is the request-driven pattern the Kafka week's Module 3 already taught as the wrong default when an
...
- A second common gap: treating `401` and `403` as interchangeable. They're not - `401` means
  ...
  this distinction right here sets up Module 9's real JWT validation guard correctly.
