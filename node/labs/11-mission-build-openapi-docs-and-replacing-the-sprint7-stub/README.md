# Lab 11 - Mission Build: OpenAPI Docs & Replacing the Existing Stub

This is the last technical lab of this week. It has two parts: documenting the auth
...
- Generated a live, browsable OpenAPI document for the auth service, continuing the Spring
  week's contract-first pattern into a second language and framework
- Completed an end-to-end integration test that proves a real token, issued by the
  NestJS auth service you have built since Module 7, authenticates a real, unmodified
  Spring Boot mission service route

## Setup

- Docker Desktop, Node.js, and a working checkout of your own Spring week mission
...
- Given, don't modify: `auth.service.ts`, `health.controller.ts`, `app.module.ts`,
  `Dockerfile` (all unchanged since Module 10/8), and everything under your own
...
written (the Spring week's Module 12/13's mechanics, reused). Four `TODO`s remain:

4. **Wait for both containers** to be ready - the same retry-loop shape as the Spring week's
...
Set `MISSION_SERVICE_DIR` to point at your own Spring week checkout before running:
